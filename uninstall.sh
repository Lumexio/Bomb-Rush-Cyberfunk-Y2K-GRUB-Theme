#!/bin/bash
# Uninstallation script for Bomb Rush Cyberfunk Y2K GRUB Theme
...
echo -e "${MAGENTA}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${MAGENTA}║  BOMB RUSH CYBERFUNK Y2K GRUB THEME UNINSTALLER         ║${NC}"
echo -e "${MAGENTA}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ -d "/boot/grub/themes" ]; then
    ...
    echo -e "${RED}GRUB themes directory not found${NC}"
    exit 1
fi
...
if [ -d "$THEME_DIR" ]; then
    echo -e "${CYAN}Removing theme directory: $THEME_DIR${NC}"
    rm -rf "$THEME_DIR"
else
...
# Remove theme line from GRUB configuration
echo -e "${CYAN}Updating GRUB configuration...${NC}"
sed -i '/^GRUB_THEME=.*bomb-rush-cyberfunk-y2k/d' "$GRUB_CONFIG"
...
echo -e "${GREEN}║  Reboot to see the default GRUB theme.                   ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
